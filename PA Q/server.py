...
def execute_java_solution(source_code, test_cases):
    ...
    # Strip package statements to prevent ClassNotFoundException due to package nesting
...
"""
    ...
    with tempfile.TemporaryDirectory(dir=temp_base, prefix="java_exec_") as tmpdir:
        ...
        # Detect target class name with public static void main
        target_class = "Main"
        match_class = re.search(r'class\s+([A-Za-z0-9_]+)[\s\S]*?public\s+static\s+void\s+main', prepared_code)
        if match_class:
