...
function initSmoothCaret(cm) {
  ...
  const linesEl = cm.getScrollerElement().querySelector(".CodeMirror-lines") || wrapper;
  ...
  function updateCaretPosition(teleport = false) {
    if (!cm) return;
  ...
  cm.on("cursorActivity", () => updateCaretPosition(false));
  cm.on("scroll", () => updateCaretPosition(false));
  cm.on("change", () => updateCaretPosition(false));
  cm.on("focus", () => {
  ...
  // Track window resize
  const resizeHandler = () => {
    if (cm.hasFocus()) updateCaretPosition(true);
  };
  window.addEventListener("resize", resizeHandler);

  smoothCaretController = {
    update: updateCaretPosition,
    teleport: () => updateCaretPosition(true),
    destroy: () => {
...
function renderTestcaseRows(cases) {
  return cases
    .map((tc) => {
      ...
      return `
      <div class="testcase-item" id="tc-item-${tcIdEsc}">
