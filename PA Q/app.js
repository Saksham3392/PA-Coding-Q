...
function saveStarredProgress() {
  try {
    localStorage.setItem(STARRED_STORAGE_KEY, JSON.stringify(Array.from(STARRED_PROBLEMS)));
  } catch (e) {
...
function isProblemStarred(prob) {
  ...
  const num = typeof prob === "object" && prob.num !== undefined ? String(prob.num).trim() : null;
  return (id && STARRED_PROBLEMS.has(String(id).trim())) || 
         (num && (STARRED_PROBLEMS.has(num) || STARRED_PROBLEMS.has(String(parseInt(num, 10)))));
}
...
function toggleProblemStarred(problemId, triggerAnimation = true) {
  ...
  showToast(nextStarred ? `★ Added "${prob.title}" to Starred list` : `☆ Removed "${prob.title}" from Starred list`);
}
...
function renderHeroStarButton(isStarred, probId, animate = false) {
  ...
  btn.setAttribute("aria-label", isStarred ? "Unstar this question" : "Star this question");
  btn.setAttribute("title", isStarred ? "Starred! Click to remove from Starred list" : "Bookmark / Star this question");
...
function renderProblemNavList() {
  ...
  if (starredPill) {
    starredPill.innerHTML = starredCount > 0 
...
function updateProgressStats() {
  ...
  const globalSolved = problemsArray.filter(p => p && solvedProblems.has(p.id)).length;

  // Identify the active category/topic of the currently selected question
  const activeProb = (currentProblemId && PROBLEMS[currentProblemId]) ? PROBLEMS[currentProblemId] : null;
  const activeTopic = activeProb ? (activeProb.category || activeProb.tag || "").trim() : null;
  ...
  if (activeTopic) {
    ...
    currentSolved = topicProblems.filter(p => solvedProblems.has(p.id)).length;

    if (progressCard) {
      progressCard.setAttribute("title", `${activeTopic}: ${currentSolved} / ${currentTotal} solved (${globalSolved} / ${totalGlobal} curriculum total)`);
    }
  } else {
    ...
    if (progressCard) {
      progressCard.setAttribute("title", `Curriculum Progress: ${currentSolved} / ${currentTotal} solved`);
    }
  }
  ...
  if (barElem) {
    const percentage = currentTotal > 0 ? Math.min(100, (currentSolved / currentTotal) * 100) : 0;
...
function setupEventListeners() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (starBtn) {
      e.preventDefault();
      e.stopPropagation();
      const probId = starBtn.getAttribute("data-problem-id") || currentProblemId;
