...
window.toggleSolutionVisibility = function(source) {
  ...
  if (source === "left" || source === "button" || !source) {
    if (leftSolContainer) {
      leftSolContainer.classList.toggle("hidden");
      const isHidden = leftSolContainer.classList.contains("hidden");
      if (leftSolBtn) {
        const span = leftSolBtn.querySelector("span");
        if (span) span.textContent = isHidden ? "Show Full Solution & Explanation" : "Hide Solution";
      }
      showToast(isHidden ? "Solution hidden" : "Solution revealed");
      return;
    }
  }

  // Keyboard shortcut Ctrl + . toggles both
  ...
  showToast(isAnyOpened ? "Solution visible" : "Solution hidden");
};
...
function setupEventListeners() {
  ...
  if (btnToggleSolution && solutionContainer) {
    btnToggleSolution.addEventListener("click", () => {
      ...
      const span = btnToggleSolution.querySelector("span");
      if (span) span.textContent = isHidden ? "Show Full Solution & Explanation" : "Hide Solution";
    });
  }
